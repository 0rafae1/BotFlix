...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (backButton) {
        backButton.addEventListener("click", () => {
            document.querySelector(".results").classList.remove("show-on-mobile");
            document.querySelector(".search-card").classList.remove("hide-on-mobile");
...
async function handleSearch() {
    ...
    if (data && data.results.length > 0) {
            ...
            if (window.innerWidth <= 600) {
                resultsDiv.classList.add("show-on-mobile");
                document.querySelector(".search-card").classList.add("hide-on-mobile");
